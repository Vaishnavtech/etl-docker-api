...
def connect_to_sql_server(request: dict):
    ...
    # Connection string
    conn_string = f"mssql+pyodbc://{username}:{password}@{server_name}"f"/master?driver=ODBC+Driver+18+for+SQL+Server""&TrustServerCertificate=yes""&Encrypt=yes"
...
async def import_data(request: ImportDataRequest):
   ...
   f"@{request.dbConfig.server}/{request.dbConfig.databaseName}"
